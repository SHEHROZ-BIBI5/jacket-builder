...
// helper: short name
const shortenName = (name) => (name ? name.split(" ")[0] : "");

const KnitBase = ({ colors, handleColorChange, isOpen, toggleDropdown }) => {
  const [selectedColor, setSelectedColor] = useState(allColors[0]);

  const handleSelectColor = (color) => {
    setSelectedColor(color);
    if (typeof handleColorChange === "function") {
      handleColorChange("knitBase", color.color);
    }
    ...
    <div className="justify-center items-center p-1 font-sans grid grid-cols-2 gap-[1px]">
      <div className="relative">
        ...
        <button
          ...
          style={{ backgroundColor: selectedColor?.color || "#868482" }}
        >
          <span
        ...
        {/* Label (hide only when dropdown is open) */}
        {!isOpen && (
        ...
        {/* Dropdown (absolute → button ki jaga fix) */}
        {isOpen && (
          <div className="absolute left-[-85px] z-50 mt-1 bg-white border border-gray-300 w-[330px]">
            {/* Icon */}
            <div className="absolute -top-[12px] left-[120px] transform -translate-x-1/2">
