...
const BodyColor = ({ colors, handleColorChange, isOpen, toggleDropdown }) => {
  const [selectedColor, setSelectedColor] = useState(allColors[0]);

  const handleSelectColor = (color) => {
    setSelectedColor(color);
    handleColorChange("body", color.color);
    ...
    <div className="justify-center items-center p-1 font-sans grid grid-cols-2 gap-[1px]">
      <div className="w-[80px] max-w-sm">
        <div className="relative">
          ...
          {!isOpen && (
            <label className="block text-sm ml-7 text-[#545c67] uppercase text-[11px] mt-[2px]">
              Body
            </label>
          )}
          ...
          {/* Dropdown */}
          {isOpen && (
            <div className="absolute z-10 w-[330px] bg-white border border-gray-300 ">
              {/* Icon */}
              <div className="absolute -top-[12px] left-[22px] transform -translate-x-1/2">
