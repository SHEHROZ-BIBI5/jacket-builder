...
const BackTopSub = ({ data = {}, setData = () => {} }) => {
  ...
  // letters, letterStyle, letterFont ab zarurat nahi hai
  ...
  const updateBackTopData = (updates) => {
    setData({
      ...data,
      ...updates,
      activeTab: activeTab,
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    updateBackTopData({ activeTab: tab });
  };
  ...
  return (
    <div className="p-2 text-sm relative">
      <div className="flex gap-1 mb-4 mt-1">
        {/* Sirf "name" aur "image" tabs render ho rahe hain */}
        {["name", "image"].map((tab) => (
          <button
            key={tab}
      ...
      {activeTab === "name" && (
        <NameTab
          ...
          setName={(val) => {
            setName(val);
            updateBackTopData({ name: val });
          }}
          font={nameFont}
          setFont={(val) => {
            setNameFont(val);
            updateBackTopData({ nameFont: val });
          }}
          textColor={textColor}
          setTextColor={(val) => {
            setTextColor(val);
            updateBackTopData({ textColor: val });
          }}
      ...
      {/* LetterTab ko hata diya gaya hai */}
