...
const BackMiddleSub = ({ data = {}, setData = () => {} }) => {
  ...
  const updateBackMiddleData = (updates) => {
    setData({
      ...data,
      ...updates,
      activeTab: activeTab,
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    updateBackMiddleData({ activeTab: tab });
  };
    ...
    <div className="p-2 text-sm relative">
      ...
      {activeTab === "name" && (
        <NameTab
          ...
          setName={(val) => {
            setName(val);
            updateBackMiddleData({ name: val });
          }}
          font={nameFont}
          setFont={(val) => {
            setNameFont(val);
            updateBackMiddleData({ nameFont: val });
          }}
          textColor={textColor}
          setTextColor={(val) => {
            setTextColor(val);
            updateBackMiddleData({ textColor: val });
          }}
      ...
      {activeTab === "image" && (
        <ImageTab
          ...
          setUploadedImage={(val) => {
            setUploadedImage(val);
            updateBackMiddleData({ image: val });
          }}
        />
      )}
